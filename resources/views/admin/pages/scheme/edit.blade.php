...
<form action="{{ route('admin.scheme.update') }}" method="POST" enctype="multipart/form-data">
    @csrf
                    ...
                    <div class="row">
                        <div class="col-lg-6">
                            <div class="form-group">
                                ...
                                <input type="hidden" name="scheme_id" value="{{$editScheme->id}}">
                                <input type="text" class="form-control" placeholder="Enter your Scheme"
                                    name="name" value="{{$editScheme->name}}">
                            </div>
                        </div>
                        <div class="col-lg-6">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" placeholder="Enter Discount %"
                                    name="discount" value="{{$editScheme->discount}}" %>
                            </div>
                        </div>
                        <div class="col-lg-6">
                            <div class="form-group">
                                ...
                                <select class="form-control" name="category[]" id="category" multiple>
                                    ...
                                    <option value="{{ $category->id }}"
                                        @foreach($editScheme->categories as $schemeCategory)
                                        {{ in_array($category->id, $editScheme->categories->pluck('cat_id')->toArray()) ? 'selected' : '' }}
                                        @endforeach
                                        >{{ $category->name }}
                                    </option>
                                    @endforeach
                        ...
                        <div class="col-lg-6">
                            <div class="form-group">
                                ...
                                <select name="course[]" id="course" class="form-control multiple" multiple>
                                    @foreach ($categories as $category)
                                    <optgroup label="{{ $category->name }}">
                                        @foreach ($category->courses()->where('status', 1)->get() as $course)
                                        <option value="{{ $course->id }}"
                                            {{ in_array($course->id, $editScheme->courseIds()) ? 'selected' : '' }}>
                                            {{ $course->title }}
                                        </option>
                                        @endforeach
                                    </optgroup>
                        ...
                        <div class="col-lg-6">
                            <div class="form-group">
                                ...
                                <select class="form-control" name="status" id="status">
                                    <option value="1" {{$editScheme->status == 1 ? 'selected' : ''}}>Active</option>
                                    <option value="0" {{$editScheme->status == 0 ? 'selected' : ''}}>Inactive</option>
                                </select>
                            </div>
                        </div>
                    </div>
...
@php
$allCourses = \App\Models\Course::where('status', 1)->get(['id', 'title', 'category_id']);
@endphp
<script>
    $('.multiple').select2();
    $('#category').select2();
